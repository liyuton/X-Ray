...
def phase1_structure_analysis(pid, year_data):
    ...
    prompt = f"""You are an expert in academic idea tree analysis. You need to analyze the evolution of idea trees across multiple years. Please pay special attention to the node color encoding:
...
Important: The edge direction in the idea tree is from source to target, meaning that the source paper is cited by the target paper. So "A → B" means paper B cites paper A.

Please analyze the idea tree data for the following years:
...
## Phase 1: Idea Tree Structural Evolution Analysis
...
def phase2_thought_analysis(pid, key_nodes, year_data):
    ...
    for node_id in key_nodes:
        ...
        print(f"Retrieved info for [key] node {node_id}: Title length = {len(title)}, Abstract length = {len(abstract)}")
...
"""
    ...
    """ Add Other Nodes Information """
    # New: extract other nodes from [all_key_edges]
    other_nodes = []
    for year, source, target in all_key_edges:
        if source not in other_nodes and source != pid and source not in key_nodes:
            other_nodes.append(source)
        if target not in other_nodes and target != pid and target not in key_nodes:
            other_nodes.append(target)
    
    ## retrieve paper info for other nodes from es
    other_node_info = {}
    for node_id in other_nodes:
        title, abstract = get_paper_info_from_es(node_id)
        other_node_info[node_id] = {"title": title, "abstract": abstract}
        print(f"Retrieved info for [other] node {node_id}: Title length = {len(title)}, Abstract length = {len(abstract)}")
    
    ## append other node info to the prompt
...
Please focus on relationships (especially multi-hop idea flow) that show significant intellectual development, such as:
...
Phase 1 Analysis Result (Structural Evolution):
{phase1_result}

Phase 2 Analysis Result (Idea Inspiration):
{phase2_result}
...
3. Highlights and analyze the most significant turning points and their intellectual implications
4. Traces and analyze the flow and path flow of key ideas through the idea tree
5. Identifies emerging trends and potential future directions
6. Please avoid using the term "citation network" and use "idea tree" instead

Please format the report into clearly sections and use academic language appropriate for researchers; ensure the structural analysis flows logically and reads naturally.
"""
...
## Executive Summary
This report analyzes the evolution of the research area centered around paper {pid}. The idea tree shows significant growth and intellectual development from 2018 to 2021.
