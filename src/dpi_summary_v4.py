...
1.  **Annual Evolution Analysis:** First, provide a **year-by-year summary** of the Idea Tree's evolution. For each year (from {min_year} to {max_year}), describe the overall changes in its structure (e.g., new nodes added, new citation links formed, key changes from the previous year).
...
"""
    ...
    for year in sorted(year_data.keys()):
        ...
        # 保护性处理并计算上限
        for pid, v in filtered_ke_data.items():
            try:
                val = float(v)
            except Exception:
                val = 0.0
    ...
    with open("../output/prompt_1113.txt", "w", encoding="utf-8") as f:
...
def main():
    ...
    with open(os.path.join(output_dir, "final_report_1113.txt"), "w", encoding="utf-8") as f:
