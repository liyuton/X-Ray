...
# Keep structure data unchanged, but reduce repeated/full abstracts in Node Information.
KEY_NODE_TOP_N = 30
KEY_ABSTRACT_CHAR_LIMIT = 500
OMITTED_NON_KEY_ABSTRACT = ""
SHARED_DUPLICATE_ABSTRACT = (
    "[Shared intersection paper; bibliographic details and abstract are provided once in Part A.]"
)
...
def truncate_abstract(abstract, limit=KEY_ABSTRACT_CHAR_LIMIT):
    if abstract is None:
        return ""
    abstract = str(abstract).strip()
    if len(abstract) <= limit:
        return abstract
    return abstract[:limit].rstrip() + "..."

def collect_involved_node_ids(year_data):
    all_node_ids = set()
    for _, data in year_data.items():
...
def collect_key_node_ids(pid, year_data, ke_data, shared_node_ids=None, top_n=KEY_NODE_TOP_N):
    """Select nodes that should keep abstracts: root, shared nodes, high-KE nodes, and central nodes."""
    all_node_ids = collect_involved_node_ids(year_data)
    key_node_ids = {str(pid)}
    if shared_node_ids:
        key_node_ids.update(str(node_id) for node_id in shared_node_ids if str(node_id) in all_node_ids)

    max_ke_by_node = {}
    for _, year_ke_data in ke_data.items():
        for node_id, raw_value in year_ke_data.items():
            node_id = str(node_id)
            ...
            try:
                ...
                continue
            if value > max_ke_by_node.get(node_id, float("-inf")):
                max_ke_by_node[node_id] = value
    ...
    top_ke_nodes = [
        node_id for node_id, _ in sorted(max_ke_by_node.items(), key=lambda item: item[1], reverse=True)[:top_n]
    ]
    top_degree_nodes = [
        node_id for node_id, _ in sorted(degree_by_node.items(), key=lambda item: item[1], reverse=True)[:top_n]
    ]
    key_node_ids.update(top_ke_nodes)
    key_node_ids.update(top_degree_nodes)
    key_node_ids.intersection_update(all_node_ids | {str(pid)})

    stats = {
        "all_nodes": len(all_node_ids | {str(pid)}),
        "key_nodes": len(key_node_ids),
        "shared_nodes": len(set(str(node_id) for node_id in (shared_node_ids or [])) & all_node_ids),
        "top_n": top_n,
        "abstract_limit": KEY_ABSTRACT_CHAR_LIMIT,
    }
    return key_node_ids, stats

# 为单个 pid 构建 node_info_str 与 structure_info_str （结构信息保持原逻辑，节点摘要做压缩）
def build_node_and_structure_strings(
    pid,
    year_data,
    ke_data,
    shared_node_ids=None,
    shared_info_by_id=None,
    duplicate_shared_ids=None,
):
    # collect unique node ids
    all_node_ids = collect_involved_node_ids(year_data)
    key_node_ids, compression_stats = collect_key_node_ids(pid, year_data, ke_data, shared_node_ids)
    duplicate_shared_ids = set(str(node_id) for node_id in (duplicate_shared_ids or []))
    shared_info_by_id = shared_info_by_id or {}

    def compressed_node_record(node_id, title, abstract, pub_year):
        node_id = str(node_id)
        is_center_paper = node_id == str(pid)
        if (not is_center_paper) and node_id in duplicate_shared_ids and node_id in shared_info_by_id:
            shared_info = shared_info_by_id[node_id]
            return {
                "id": node_id,
                "title": shared_info.get("title", title),
                "abstract": SHARED_DUPLICATE_ABSTRACT,
                "publication_year": shared_info.get("publication_year", pub_year),
            }
        if node_id in key_node_ids:
            return {
                "id": node_id,
                "title": title,
        ...
        return {
            "id": node_id,
            "title": title,
            "abstract": OMITTED_NON_KEY_ABSTRACT,
            "publication_year": pub_year,
        }
    ...
    try:
        ...
        root_record = compressed_node_record(pid, title, abstract, pub_year)
        root_paper_info = (
            f"Center paper for analysis\n"
            f"Title: {title}\n"
            f"Year: {pub_year}\n"
            f"Abstract: {root_record['abstract']}\n"
        )
        node_info_list.append(root_record)
    ...
    for node_id in sorted(all_node_ids):
        if node_id == str(pid):
            continue
        try:
            if node_id in duplicate_shared_ids and node_id in shared_info_by_id:
                shared_info = shared_info_by_id[node_id]
                title = shared_info.get("title", f"Paper_{node_id}")
                abstract = shared_info.get("abstract", "")
                pub_year = shared_info.get("publication_year", "Unknown")
            else:
                title, abstract, pub_year = get_paper_info_from_es(node_id)
            node_info_list.append(compressed_node_record(node_id, title, abstract, pub_year))
        except Exception as e:
            print(f"Error fetching info for node {node_id}: {e}")
            node_info_list.append(
                compressed_node_record(
                    node_id,
                    f"Paper_{node_id}",
                    "Abstract not available or fetching error.",
                    "Unknown",
                )
            )
    node_info_str = "## Node Information (For your abstract analysis)\n\n"
    node_info_str += (
        "Compression note: key nodes keep abstracts truncated to "
        f"{KEY_ABSTRACT_CHAR_LIMIT} characters; non-key node abstracts are omitted; "
        "duplicate shared intersection papers in Part B point back to their Part A records.\n"
        f"Compression stats: {json.dumps(compression_stats, ensure_ascii=False)}\n\n"
    )
    ...
    for year in sorted(year_data.keys()):
        ...
        structure_info_str += "\n"
    return node_info_str, structure_info_str, node_info_list, compression_stats
...
The Node Information, Idea Tree Structure, and VD/DPI Annual Evidence data for each PID are provided below as separate sections. Use ONLY this information when making claims. Do NOT invent abstracts, citations, or historical facts. Some Node Information abstracts are intentionally compressed: an empty abstract means the paper is a non-key structural node, and a shared-duplicate note means the same intersection paper's full compressed record appears once in Part A.
...
def main(pid1, pid2):
    ...
    # Shared papers are kept once with compressed abstracts in Part A; Part B references them.
    shared_node_ids = collect_involved_node_ids(year_data_1) & collect_involved_node_ids(year_data_2)
    print(f"Shared intersection nodes detected: {len(shared_node_ids)}")

    node_info_1, struct_info_1, node_records_1, stats_1 = build_node_and_structure_strings(
        pid1,
        year_data_1,
        ke_data_1,
        shared_node_ids=shared_node_ids,
    )
    shared_info_by_id = {record["id"]: record for record in node_records_1 if record["id"] in shared_node_ids}
    node_info_2, struct_info_2, node_records_2, stats_2 = build_node_and_structure_strings(
        pid2,
        year_data_2,
        ke_data_2,
        shared_node_ids=shared_node_ids,
        shared_info_by_id=shared_info_by_id,
        duplicate_shared_ids=shared_node_ids,
    )
    print(f"Compression stats for pid {pid1}: {stats_1}")
    print(f"Compression stats for pid {pid2}: {stats_2}")
    ...
    prompt_path = os.path.join(output_dir, "prompt_v4.txt")
    ...
    # Save final report
    report_path = os.path.join(output_dir, "final_report_v4.txt")
