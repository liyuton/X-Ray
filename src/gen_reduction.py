...
#date  : 20200711
#=====================================================================================
# 邻接矩阵是稠密矩阵；特征分解是全量稠密；最短路在深层循环里反复调用单点 dijkstra；在大图上非常慢
...
def gen_reduction(paper_id, INPUT_FILE_PATH):
    ...
    print(INPUT_FILE_PATH, '>>> LaplacianMatrix calculated!')
    EigenValue,EigenVector = getKSmallestEigVec(LaplacianMatrix,len(NodeIDList)) # 特征分解是全量稠密 eig
    print(INPUT_FILE_PATH, '>>> EigenVector calculated!')
            ...
            while (not(MyQueueNext.empty())):
                ...
                while (not(MyQueueNow.empty())):
                    ...
                    if NodeOperateNow.ReturnID() in Distance3Index_flag:
                        ...
                        # 使用 point-to-point 单源最短路，每次调用只计算从一个源点到一个目标点的路径
