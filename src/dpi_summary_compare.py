...
You are an expert research domain analyst. Your task is to produce a single, coherent analysis report in English comparing the Idea Tree evolution of two target papers.

## Core Concepts (definitions you must use)
* **Idea Tree:** directed graph showing knowledge evolution centered on a target paper. Nodes = papers; edges = citation relationships.
* **Edge Definition:** Edge (A -> B) means paper A is cited by paper B, indicating knowledge flowed from A to B.
* **Knowledge Flow:** directional influence where A inspired or contributed methods/ideas to B.
* **Knowledge Entropy:** numeric metric indicating how diverse the knowledge sources feeding into a node are; high values imply integration from diverse directions and potential innovation.
* **Key Historical Intersection Points (under Idea Tree guidance):** moments (nodes, edges, or time slices) where two or more research threads converge, generating cross-fertilization or major shifts.

## Task overview (strict)
You must produce ONE single English report that contains:
1. A **brief standalone analysis** for **each** target paper (PID {pid1} and PID {pid2}). Each standalone analysis should:
   - Summarize the Idea Tree evolution for that PID (focusing on years present).
   - Identify 2–4 key nodes / hubs (prefer nodes with high Knowledge Entropy).
   - Explain concrete knowledge flow paths using the paper abstracts from the Node Information sections (i.e., show how one paper inspired another by quoting or paraphrasing from the abstracts).
   - Keep each standalone analysis concise (approx. 2–4 paragraphs each).

2. A **comparative analysis** section (after the two standalone analyses) that:
   - Focuses on **historical intersection points** between the two Idea Trees (where the two theme trees intersect or share influences).
   - Uses Knowledge Entropy, shared nodes, shared citation sources, and the structural evolution to surface intersection points.
   - For each intersection point, explain why it is historically important and how ideas flowed across the two trees (cite paper titles or short forms and publication years).
   - **If you find concrete intersection points between the two Idea Trees (i.e., specific papers or nodes that lie on both trees or act as bridges), you MUST return the full bibliographic information for those intersecting papers.** For each intersecting paper provide: Title, Authors (if available in the Node Information), Publication year, and any identifier available (e.g., PID or OpenAlex ID). Place these details in a clearly labeled subsection titled **"Identified Intersection Papers"** inside the Comparative Historical Intersection Analysis. If the same paper appears as an intersection for multiple years or multiple edges, list it once and note the corresponding years/edges/trees where it is referenced.
   - Highlight differences in thematic evolution, methodology, and knowledge integration patterns between the two PIDs.
   - Provide a short bullet list of 3–6 actionable insights or hypotheses for future research that arise from the comparative historical intersection analysis.

3. **Report format requirements**
   - Use clear section headings: "Standalone Analysis — PID {pid1}", "Standalone Analysis — PID {pid2}", "Comparative Historical Intersection Analysis", "Insights", and "Reference List".
   - Do not expose raw IDs in the narrative; use paper titles or short forms.
   - At the end include a `## Reference List` listing all papers you explicitly mention, formatted as:
     * [n] Title. (Year)

4. IMPORTANT: The Node Information and Idea Tree Structure data for each PID are provided below as separate sections. Keep them separated: first pid1's Node Info & Structure, then pid2's Node Info & Structure. Use them directly when making assertions (do not invent details not present in the abstracts).

Now proceed. First produce the two short standalone analyses (one for each PID), then the comparative analysis focusing on theme-tree-guided historical intersection points, then actionable insights, and finally the Reference List.
"""
