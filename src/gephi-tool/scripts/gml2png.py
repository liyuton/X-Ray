...
sys.path.insert(0, '/home/liyutong1117/jupyter/scientific_x_ray-github/src')
...
def gml2png(arg):
    global config
    gml_input_path = arg[0]
    png_output_path = arg[1]
    bg_type = config['background']
    im_size = 512
    ...
    for node in graph['nodes']:
        ...
        x2 = ((graph['nodes'][node]['x'] + r) - bbox[0]) / (bbox[2] - bbox[0]) * im_w + (im_s - im_w) / 2
        y2 = ((graph['nodes'][node]['y'] + r) - bbox[1]) / (bbox[3] - bbox[1]) * im_h + (im_s - im_h) / 2
        font_size = int((x2 - x1) / 2.7)  # 待定
        script_path = os.path.split(os.path.realpath(__file__))[0]
        font_source_path = os.path.join(os.path.split(script_path)[0], 'font')
        font = ImageFont.truetype(os.path.join(font_source_path, config.get('font', 'arial.ttf')), font_size)
        ...
        if w / 2 < min_d:
            if bg_type == 1:
                draw.text(text_start, graph['nodes'][node]['label'], fill="#ffffff", font=font)
            elif bg_type == 0:
            ...
            text_start = [(x1 + x2) / 2 - w / 2, (y1 + y2) / 2 - h / 2]
            if bg_type == 1:
                draw.text(text_start, label, fill="#ffffff", font=font)
            elif bg_type == 0:
                draw.text(text_start, label, fill="#000000", font=font)
    im.thumbnail((im_s / 4, im_s / 4))
...
def main():
    if len(sys.argv) != 4 and len(sys.argv) != 6:
        print(
            'Usage: python gml2png.py config.json input.png output.png\n'
            'Or: python gml2png.py config.json -d input_dir output_dir [process_count=1]'
        )
        exit(1)
    if len(sys.argv) == 4:
        load_config(sys.argv[1])
        gml2png((sys.argv[2], sys.argv[3]))
    if len(sys.argv) == 6 and sys.argv[2] == '-d':
        work_list = []
        for file_name in tqdm(os.listdir(sys.argv[3])):
            if os.path.splitext(file_name)[1] == '.gml':
                work_list.append((os.path.join(sys.argv[3], file_name), os.path.join(sys.argv[4], file_name.split('.')[0] + '.jpg')))
        process_count = int(sys.argv[5])
        with Pool(process_count, initializer=load_config, initargs=(sys.argv[1], )) as pool:
            pool.map(gml2png, work_list)
